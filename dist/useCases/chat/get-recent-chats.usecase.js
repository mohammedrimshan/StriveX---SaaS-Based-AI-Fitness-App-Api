"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.GetRecentChatsUseCase = void 0;
const tsyringe_1 = require("tsyringe");
const custom_error_1 = require("@/entities/utils/custom.error");
const constants_1 = require("@/shared/constants");
let GetRecentChatsUseCase = class GetRecentChatsUseCase {
    constructor(_messageRepository, _clientRepository, _trainerRepository) {
        this._messageRepository = _messageRepository;
        this._clientRepository = _clientRepository;
        this._trainerRepository = _trainerRepository;
    }
    execute(userId, limit) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!userId) {
                throw new custom_error_1.CustomError(constants_1.ERROR_MESSAGES.ID_NOT_PROVIDED, constants_1.HTTP_STATUS.BAD_REQUEST);
            }
            if (limit < 1) {
                throw new custom_error_1.CustomError("Invalid limit parameter", constants_1.HTTP_STATUS.BAD_REQUEST);
            }
            // Fetch the requesting user's details
            let currentUserName = "Unknown";
            let currentUserAvatar = "";
            let currentUserStatus = "offline";
            const currentClient = yield this._clientRepository.findByClientId(userId);
            if (currentClient) {
                currentUserName = `${currentClient.firstName} ${currentClient.lastName}`;
                currentUserAvatar = currentClient.profileImage || "";
                currentUserStatus = currentClient.isOnline ? "online" : "offline";
            }
            else {
                const currentTrainer = yield this._trainerRepository.findById(userId);
                if (currentTrainer) {
                    currentUserName = `${currentTrainer.firstName} ${currentTrainer.lastName}`;
                    currentUserAvatar = currentTrainer.profileImage || "";
                    currentUserStatus = currentTrainer.isOnline ? "online" : "offline";
                }
            }
            const chats = yield this._messageRepository.getRecentChats(userId, limit);
            return yield Promise.all(chats.map((chat) => __awaiter(this, void 0, void 0, function* () {
                const otherUserId = chat.userId;
                let participantName = "Unknown";
                let participantAvatar = "";
                let participantStatus = "offline";
                const client = yield this._clientRepository.findByClientId(otherUserId);
                if (client) {
                    participantName = `${client.firstName} ${client.lastName}`;
                    participantAvatar = client.profileImage || "";
                    participantStatus = client.isOnline ? "online" : "offline";
                }
                else {
                    const trainer = yield this._trainerRepository.findById(otherUserId);
                    if (trainer) {
                        participantName = `${trainer.firstName} ${trainer.lastName}`;
                        participantAvatar = trainer.profileImage || "";
                        participantStatus = trainer.isOnline ? "online" : "offline";
                    }
                }
                return {
                    userId: otherUserId,
                    lastMessage: chat.lastMessage,
                    unreadCount: chat.unreadCount,
                    participants: [
                        {
                            id: userId,
                            name: currentUserName,
                            avatar: currentUserAvatar,
                            status: currentUserStatus,
                        },
                        {
                            id: otherUserId,
                            name: participantName,
                            avatar: participantAvatar,
                            status: participantStatus,
                        },
                    ],
                };
            })));
        });
    }
    determineRole(userId) {
        return __awaiter(this, void 0, void 0, function* () {
            const client = yield this._clientRepository.findByClientId(userId);
            if (client)
                return constants_1.ROLES.USER;
            const trainer = yield this._trainerRepository.findById(userId);
            if (trainer)
                return constants_1.ROLES.TRAINER;
            throw new custom_error_1.CustomError("User not found", constants_1.HTTP_STATUS.NOT_FOUND);
        });
    }
};
exports.GetRecentChatsUseCase = GetRecentChatsUseCase;
exports.GetRecentChatsUseCase = GetRecentChatsUseCase = __decorate([
    (0, tsyringe_1.injectable)(),
    __param(0, (0, tsyringe_1.inject)("IMessageRepository")),
    __param(1, (0, tsyringe_1.inject)("IClientRepository")),
    __param(2, (0, tsyringe_1.inject)("ITrainerRepository")),
    __metadata("design:paramtypes", [Object, Object, Object])
], GetRecentChatsUseCase);
