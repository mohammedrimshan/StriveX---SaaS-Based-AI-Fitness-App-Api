...
import { IAiWorkoutPlanRepository, IAiDietPlanRepository } from "@/entities/repositoryInterfaces/client/ai-plan-repository";
...
@injectable()
export class AiWorkoutPlanRepository extends BaseRepository<IWorkoutPlan> implements IAiWorkoutPlanRepository {
  constructor() {
    super(WorkoutPlanModel);
  }

  async findByClientId(clientId: string): Promise<IWorkoutPlan[]> {
    const plans = await this.model.find({ clientId }).sort({ createdAt: -1 }).lean();
  ...
  async update(id: string, plan: Partial<IWorkoutPlan>): Promise<IWorkoutPlan | null> {
    ...
    const updatedPlan = await this.model
      .findByIdAndUpdate(id, { $set: { ...plan, updatedAt: new Date() } }, { new: true })
      .lean();
...
@injectable()
export class AiDietPlanRepository extends BaseRepository<IDietPlan> implements IAiDietPlanRepository {
  constructor() {
    super(DietPlanModel);
  }
