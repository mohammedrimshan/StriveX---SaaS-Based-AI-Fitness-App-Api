...
@injectable()
export class CommentRepository extends BaseRepository<ICommentEntity> implements ICommentRepository {
  constructor() {
    super(CommentModel);
  }

  async findByPostId(postId: string, skip: number, limit: number): Promise<{ items: ICommentEntity[]; total: number }> {
  ...
  async addLike(commentId: string, userId: string): Promise<ICommentEntity | null> {
  ...
  async removeLike(commentId: string, userId: string): Promise<ICommentEntity | null> {
    const comment = await this.model
  ...
  async addReport(commentId: string, report: ICommentEntity['reports'][0]): Promise<ICommentEntity | null> {
    const comment = await this.model
  ...
  async findReportedComments(): Promise<ICommentEntity[]> {
    const comments = await this.model.find({ 'reports.0': { $exists: true } }).lean();
